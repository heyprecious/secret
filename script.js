(function () {
  ...
  // Balloon interval for endless spawning
  let balloonInterval = null;

  const EXIT_URL = 'https://websitecursor.com/downloads';
  const COUNTDOWN_STORAGE_KEY = 'birthdayCountdownStartTime';
  const CANDLES_BLOWN_KEY = 'candlesBlown'; // Track if candles have been blown

  function showScreen(screenEl) {
    // Stop balloons if leaving message screen
    if (messageScreen && messageScreen.classList.contains('active') && screenEl !== messageScreen) {
      stopEndlessBalloons();
    }
    ...
    if (screenEl === cakeScreen) {
      ...
      const candlesBlown = localStorage.getItem(CANDLES_BLOWN_KEY) === 'true';
      const countdownDisplay = document.getElementById('countdown-display');
      
      const countdownTitle = document.querySelector('.countdown-title');
      
      const countdownWrapper = document.querySelector('.countdown-wrapper');
      // Show countdown if candles were blown (saved start time exists)
      // This persists even after page refresh, browser close, or website clear (if localStorage persists)
      if (savedStartTime && candlesBlown) {
        // Resume countdown if there's a saved time - candles were already blown
        // This ensures the countdown stays visible even after leaving/returning
        if (!countdownInterval) {
          startCountdown(); // Start countdown first to ensure time is calculated
        }
        if (countdownWrapper) {
  ...
  function initializeEventListeners() {
    ...
    if (btnOpenCakeSelection) {
      btnOpenCakeSelection.addEventListener('click', () => {
        ...
        // showScreen() will handle showing/hiding countdown based on saved state
        // Don't manually hide it here as it might override the restoration logic
      });
    }
    ...
    if (btnExitMessage) {
      btnExitMessage.addEventListener('click', () => {
        stopEndlessBalloons();
        exitToUrl();
      });
    }
    ...
    if (clickableAsterisk && resetTimerBtn) {
      ...
      // Reset countdown to 365 days from midnight of today
      const now = new Date();
      const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
      countdownStartTime = startOfToday.getTime();
  ...
  function blowOutCandles() {
    ...
    if (blowingDetected) {
      ...
      if (candles.every((candle) => candle.classList.contains("out"))) {
        ...
        // Show and start countdown immediately (no delay)
        // Mark that candles have been blown FIRST (persists even after page refresh/clear)
        localStorage.setItem(CANDLES_BLOWN_KEY, 'true');
        startCountdown(); // Start countdown - this will save the start time if not already saved
        const countdownWrapper = document.querySelector('.countdown-wrapper');
        const countdownDisplay = document.getElementById('countdown-display');
        const countdownTitle = document.querySelector('.countdown-title');
        if (countdownWrapper) {
  ...
  function spawnBalloons(count) {
    if (!balloonsLayer) return;
    // Clear existing balloons only when spawning initial batch
  ...
  function startEndlessBalloons() {
    ...
    // Clear existing balloons and start fresh
    if (balloonsLayer) {
      balloonsLayer.innerHTML = '';
    }
    
    // Spawn initial batch of balloons
    for (let i = 0; i < 20; i++) {
      spawnBalloon();
    }
    
    // Continuously spawn new balloons every 1-2 seconds
    balloonInterval = setInterval(() => {
      spawnBalloon();
    }, 800 + Math.random() * 800); // Random interval between 800ms and 1600ms
  }

  function stopEndlessBalloons() {
    if (balloonInterval) {
      clearInterval(balloonInterval);
  ...
  function startCountdown() {
    ...
    if (savedStartTime) {
      ...
      // Uses device's actual time to calculate remaining time accurately
      countdownStartTime = parseInt(savedStartTime, 10);
      console.log("Resuming countdown from saved time:", new Date(countdownStartTime));
    } else {
      // Start new countdown from midnight (12am) of the birthday
      // This ensures the countdown starts from the beginning of the birthday day
      // If candles are blown at 9am, it will show 364 days and ~15 hours (since 9 hours passed from midnight)
      const now = new Date();
      const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 0, 0, 0, 0);
      countdownStartTime = startOfToday.getTime();
      localStorage.setItem(COUNTDOWN_STORAGE_KEY, countdownStartTime.toString());
      console.log("Starting new 365-day countdown from birthday (midnight):", new Date(countdownStartTime));
      console.log("Countdown starts from 12am of the birthday day");
    }
    
    // Update immediately using device's current time (no delay)
    updateCountdown();
    
    // Update every second using device's current time (synchronized with device clock)
    countdownInterval = setInterval(updateCountdown, 1000);
  }
  ...
  function updateCountdown() {
    ...
    // Always use device's actual current time (synchronized with device clock)
    // This ensures the countdown is always accurate and not delayed or ahead
    const now = Date.now();
    const elapsed = now - countdownStartTime;
    const remaining = Math.max(0, COUNTDOWN_DURATION - elapsed);
    
    // Calculate days, hours, minutes, seconds (365 days total)
    // Using device's current time ensures accuracy
    ...
    if (remaining <= 0) {
      ...
      localStorage.removeItem(CANDLES_BLOWN_KEY);
